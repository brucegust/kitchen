<?php
if($_POST['frequency']=="every month")
{
$frequency_title="every month";
...
//right here we're going to ask the question if the "all day" check mark has been checked
	...
	if($all_day>0)
	{
	 while ($the_start<= $the_event_end_repeat) 
		...
		$event_start = date('Y-m-d H:i:s', strtotime("$event_start + 1 month"));
		$the_start = strtotime('+1 month', $the_start);
		}
	}
	else
	{
	while ($the_start<= $the_event_end_repeat) 
		...
		$event_start = date('Y-m-d H:i:s', strtotime("$event_start + 1 month"));
		$event_end= date('Y-m-d H:i:s', strtotime("$event_end + 1 month"));
		$the_start = strtotime('+1 month', $the_start);
		}
	}
}
