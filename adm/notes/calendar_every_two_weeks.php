<?php
if($_POST['frequency']=="every two weeks")
{
$frequency_title="every two weeks";
...
//right here we're going to ask the question if the "all day" check mark has been checked
	...
	while ($the_start<= $the_event_end_repeat) 
		...
		$event_start = date('Y-m-d H:i:s', strtotime("$event_start + 14 day"));
		$the_start = strtotime('+2 week', $the_start);
		}
	}
	else
	{
	while ($the_start<= $the_event_end_repeat) 
		...
		$event_start = date('Y-m-d H:i:s', strtotime("$event_start + 14 day"));
		$event_end= date('Y-m-d H:i:s', strtotime("$event_end + 14 day"));
		$the_start = strtotime('+2 week', $the_start);
		}
	}
}
