...
function shopper_alert($shopper_id) {

	global $cxn;

	$sql="select DISTINCT cart.door_id, doors.id as door_table_id, features.featurename as doorname FROM cart INNER JOIN features ON cart.door_id = features.part_number INNER JOIN doors on doors.feature_door_id=cart.door_id WHERE cart.session_id='$shopper_id'";
